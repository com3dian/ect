"""Load Gemma-2 + Gemma Scope SAE; extract SAE features and target log-probs."""
...
def extract_sae_features(
    ...
    *,
    ...
    """Residual at ``layer_index`` (final token) → SAE encode. CPU float32 [d_sae]."""
    ...
    hs_index = hidden_state_index(layer_index)
...
def target_token_ids(tokenizer: Any, text: str, *, max_tokens: int = 8) -> list[int]:
...
def mean_target_logprob(
    ...
    *,
    ...
    """
    Mean log π(target tokens | prompt): tropical d_M = -this value.

    Scores each target subword against logits immediately before it.
    """
