"""
Rank-aligned probability differences: min / mean / max vs empirical P_{A∨B}.

For each pair:
  1. Sort P_{A∨B} Top-K tokens by probability (high → low).
  2. At those tokens, evaluate pointwise min / mean / max of (p_A, p_B)
     (missing Top-K mass treated as 0).
  3. Record absolute difference |agg(token) − P_{A∨B}(token)| at each rank.
...
def _topk_maps(row: Mapping[str, Any]) -> tuple[dict[int, float], dict[int, float], list[int], np.ndarray]:
    """Return p_A map, p_B map, P_{A∨B} token ids (unsorted), P_{A∨B} probs (unsorted)."""
    ...
    or_ids = [int(t) for t in row["P_A_or_B"]["token_ids"]]
    or_probs = np.asarray(row["P_A_or_B"]["probs"], dtype=np.float64)
    return a, b, or_ids, or_probs


def pair_rank_differences(row: Mapping[str, Any]) -> dict[str, np.ndarray]:
    """
    Sort empirical P_{A∨B} high→low, then return absolute diffs for min/mean/max.

    Each array has shape (K,) with K = len(P_{A∨B} Top-K).
    """
    a_map, b_map, or_ids, or_probs = _topk_maps(row)
    order = np.argsort(-or_probs, kind="mergesort")
    tids = [or_ids[i] for i in order]
    p_or = or_probs[order]
    ...
    return {name: np.abs(vec - p_or) for name, vec in aggs.items()}
...
def plot_category_rank_diffs(
    ...
    *,
    ...
    X = token rank by P_{A∨B} (0 = highest prob).
    Y = absolute difference |agg − P_{A∨B}|.
    ...
    for cid in sorted(summary):
        ...
        fig.suptitle(
            ...
            "x = P_{A∨B} rank (high→low)   y = |agg − P_{A∨B}|   ribbon = 5%–95% quantile",
            fontsize=11,
        )
        for ax, agg in zip(axes, AGG_NAMES):
            ...
            ax.set_title(agg)
            ax.set_xlabel("token rank (P_{A∨B} high → low)")
            if log_x:
                ax.set_xscale("log")
                ax.set_xlim(1, k)
