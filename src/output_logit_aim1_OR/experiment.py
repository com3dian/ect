"""
Incremental OR extraction + colimit metrics.

For each cached (P_A, P_B) row from the AND dataset:
  1) extract only P_{A∨B} via the disjunction prompt
  2) merge into the OR distributions JSONL
...
from .model_io import TopKDistribution, extract_topk_at_final_token, load_model_and_tokenizer
...
def run_experiment(
    *,
    cached_distributions_jsonl: Path | str = DEFAULT_CACHED_DISTRIBUTIONS_JSONL,
    model_id: str = MODEL_ID,
    top_k: int = TOP_K,
    device: str = DEVICE,
    dtype_name: str = DTYPE,
    distributions_jsonl: Path | str = DEFAULT_DISTRIBUTIONS_JSONL,
    metrics_jsonl: Path | str = DEFAULT_METRICS_JSONL,
    ...
    """
    Incremental OR extraction loop.

    Only P_{A∨B} is forwarded through the model; P_A and P_B are inherited
    from the cached AND distributions file.
    """
    ...
    if not todo:
        ...
        return {"n_total": len(cached_rows), "n_done": len(done), "n_new": 0}
    ...
    for i, row in enumerate(todo, start=1):
        ...
        dist_or = extract_topk_at_final_token(
            tokenizer, model, prompt_or, device=device_t, top_k=top_k
        )
        ...
        if verbose and (i == 1 or i % 10 == 0 or i == len(todo)):
            print(
                f"[{i}/{len(todo)}] cat={meta['category_id']} "
                f"{word_a!r} ∨ {word_b!r} | "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract P_{A∨B} distributions and score vs max(P_A, P_B)."
    )
    parser.add_argument(
        "--cached-distributions",
        type=Path,
        default=DEFAULT_CACHED_DISTRIBUTIONS_JSONL,
        help="JSONL with cached P_A / P_B from the AND run.",
    )
    parser.add_argument(
        "--distributions-jsonl",
        type=Path,
        default=DEFAULT_DISTRIBUTIONS_JSONL,
        help="Output JSONL for merged OR distributions.",
    )
    parser.add_argument(
        "--metrics-jsonl",
        type=Path,
        default=DEFAULT_METRICS_JSONL,
    )
    parser.add_argument(
        "--metrics-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional category_id filter (e.g. --categories 4 5).",
    )
