...
def num_transformer_layers(model: Any) -> int:
    ...
    if n is not None:
        return int(n)
    # Fallback: probe with a tiny forward if config is missing.
...
def _final_norm_module(model: Any) -> Any | None:
    ...
    # Qwen / LLaMA-style: model.model.norm
...
def hidden_to_logits(
    ...
    *,
    ...
    `hidden` is a single residual-stream vector [H] or a batch [B, H].
    """
    h = hidden
    if apply_final_norm:
...
def extract_topk_logit_lens(
    ...
    *,
    ...
    # hidden_states: (embed, after_block_0, ..., after_block_{n-1})
    ...
    # Final prompt token residual stream at the chosen layer.
