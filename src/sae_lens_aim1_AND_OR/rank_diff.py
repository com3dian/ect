"""
Rank-aligned SAE feature differences: min / mean / max vs empirical V_AND / V_OR.

Mirrors output_logit / logit_lens rank_diff plots, but over SAE features:

For each pair and operator op ∈ {and, or}:
  1. Sort empirical V_op Top-K features by mass (high → low).
  2. At those feature ids, evaluate pointwise min / mean / max of (V_A, V_B)
     (missing Top-K mass treated as 0).
  3. Record |agg(feature) − V_op(feature)| at each rank.
...
def plot_category_rank_diffs(
    ...
    *,
    ...
    emp_label = "V_AND" if op == "and" else "V_OR"
