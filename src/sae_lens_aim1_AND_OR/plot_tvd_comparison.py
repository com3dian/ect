"""
Plot SAE composition TVD: min / mean / max vs empirical AND and OR.

For each orthogonality category:
  - Top row:  TVD(V_AND, theory) for theory in {min, mean, max}
  - Bottom:   TVD(V_OR,  theory) for theory in {min, mean, max}
    ...
    *,
    ...
    for cid in sorted(summary):
        ...
        fig.suptitle(
            ...
            "y = TVD(empirical, theory)   boxes = pair distribution",
            fontsize=11,
        )
...
def plot_summary_bars(
    ...
    *,
    ...
    fig.suptitle(
        f"{layer_tag}SAE composition TVD by category\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot AND/OR TVD vs min/mean/max from SAE metrics CSV."
    )
