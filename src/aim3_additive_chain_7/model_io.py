"""Model I/O: sequence log π and BPE length helpers (v7)."""
...
def concept_bpe_len(tokenizer: Any, concept: str) -> int:
    """BPE length of a concept under the same leading-space convention as scoring."""
    return len(target_token_ids(tokenizer, concept, max_tokens=64))


def nodes_within_bpe_budget(
    tokenizer: Any,
    nodes: tuple[str, str, str, str],
    *,
    max_bpe: int,
) -> bool:
    return all(concept_bpe_len(tokenizer, n) <= int(max_bpe) for n in nodes)
...
def sequence_target_logprob(
    ...
    *,
    ...
    target_as_raw: bool = False,
) -> dict[str, float | int]:
    """
    Sum log π(target tokens | prefix).

    If ``target_as_raw``, encode ``target_text`` as-is (no leading-space rewrite),
    used for multi-line joint continuations.
    """
    if target_as_raw:
        ids = tokenizer.encode(target_text, add_special_tokens=False)
        target_ids = [int(t) for t in ids[: max(1, int(max_tokens))]] if ids else []
    else:
