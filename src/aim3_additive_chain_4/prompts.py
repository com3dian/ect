"""
Shared-trajectory prompts for true -ln π distances.

Prefix through node i = text[:char_end of node i].
Hop i→j scores label j as a continuation of that prefix (label jump, not
scoring the intervening narrative — so chain-rule tautology does not apply).
"""
...
class TrajectoryPrompt:
    ...
    def prefix_through(self, waypoint_index: int) -> str:
        """Text up to and including waypoint ``waypoint_index`` (0-based)."""
        _, end = self.char_spans[int(waypoint_index)]
...
def type_a_pairs(traj: TrajectoryPrompt) -> list[dict[str, Any]]:
    """
    Directed (prefix_waypoint, target_label) pairs for true -ln π.

    Hops use consecutive nodes; shortcut jumps n1 → n4 label.
    """
    return [
        {"pair_name": "n1_n2", "prefix_idx": 0, "target_idx": 1},
        {"pair_name": "n2_n3", "prefix_idx": 1, "target_idx": 2},
        {"pair_name": "n3_n4", "prefix_idx": 2, "target_idx": 3},
        {"pair_name": "n1_n4", "prefix_idx": 0, "target_idx": 3},
    ]
