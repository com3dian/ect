...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Plot min / mean / max differential SAE feature-rank diffs vs "
            "V'_AND and V'_OR (three panels per category)."
        )
    )
    ...
    if not distributions.exists():
        raise SystemExit(
            ...
            "Run: python -m sae_lens_aim1_AND_OR_baseline_substraction.experiment"
        )
