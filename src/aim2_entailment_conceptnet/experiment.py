"""
Aim-2 entailment experiment on ConceptNet pairs: vector extraction + patching sweep.

Same pipeline as ``aim2_entailment``; default pairs from ConceptNet IsA / PartOf JSONL.
"""
            ...
            for vector_type, key in (("trad", "v_trad"), ("ect", "v_ect")):
                ...
                for alpha in alphas:
                    ...
                    rec = {
                        ...
                        "conceptnet_edge": row.get("conceptnet_edge"),
                        "layer_index": resolved,
                        "alpha": float(alpha),
...
def run_experiment(
    *,
    ...
    if verbose:
        ...
        print(f"pairs={pairs_path} (n={len(pairs)}, source=conceptnet)")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aim-2 entailment (ConceptNet): Jacobian vectors + activation patching."
    )
