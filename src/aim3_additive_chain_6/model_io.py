"""Model I/O: sequence log π(target tokens | prefix) for Markov cloze."""
...
def target_token_ids(
    ...
    """Leading-space continuation (typical BPE continuation after cloze stem)."""
    text = text.strip()
...
def sequence_target_logprob(
    ...
    *,
    ...
    """
    Sequence log π(target subwords | prefix) = sum_i log π(t_i | prefix, t_<i).

    This is the proper multi-token conditional for d_M = -ln π(y|x).
    Also returns mean and first-token logprob for diagnostics.
    """
