"""Aim-3 v6: Markov cloze factorization with true sequence log π (Type A)."""
...
# Comma-separated: natural,shuffled_middles,reversed
...
def distances_jsonl(layer_index: int, base: Path | str = DEFAULT_OUTPUT_DIR) -> Path:
    return layer_output_dir(layer_index, base) / "markov_ln_distances.jsonl"


def summary_json(layer_index: int, base: Path | str = DEFAULT_OUTPUT_DIR) -> Path:
    return layer_output_dir(layer_index, base) / "markov_ln_summary.json"
