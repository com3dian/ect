...
def resolve_dtype(dtype_name: str = DTYPE, device: torch.device | None = None) -> torch.dtype:
    ...
    dtype = mapping[name]
    # CPU often lacks bf16 kernels; fall back to float32.
...
def load_model_and_tokenizer(
    ...
    # Some Qwen tokenizers have no pad token; reuse eos for batching safety.
...
def extract_topk_at_final_token(
    ...
    *,
    ...
    The README specifies the colon ":" as the final token of the typicality
    template; we always read whatever token is last after tokenization.
    """
    ...
    # Logits shape: [batch, seq, vocab]. Take final position only.
