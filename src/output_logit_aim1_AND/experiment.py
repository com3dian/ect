...
For each (word_a, word_b):
  ...
  4) append sparse distributions + metrics to disk (resumable)
"""
...
from .model_io import extract_topk_at_final_token, load_model_and_tokenizer
...
def _rewrite_metrics_csv(metrics_jsonl: Path, metrics_csv: Path) -> None:
    ...
    if not rows:
        return
    # Flatten: metadata + metric floats only (skip nested blobs).
    fieldnames: list[str] = []
...
def run_experiment(
    *,
    ...
    if verbose:
        ...
        print(f"corpus={corpus_path} (n={len(corpus)}, todo={len(todo)}, done={len(done)})")
        print(f"top_k={top_k} device={device} dtype={dtype_name}")

    if not todo:
        _rewrite_metrics_csv(metrics_jsonl, metrics_csv)
        return {"n_total": len(corpus), "n_done": len(done), "n_new": 0}
    ...
    for i, row in enumerate(todo, start=1):
        ...
        # Three forward passes: concept A, concept B, composition A∧B.
        dist_a = extract_topk_at_final_token(
            tokenizer, model, prompts["P_A"], device=device_t, top_k=top_k
        )
        dist_b = extract_topk_at_final_token(
            tokenizer, model, prompts["P_B"], device=device_t, top_k=top_k
        )
        dist_ab = extract_topk_at_final_token(
            tokenizer, model, prompts["P_AB"], device=device_t, top_k=top_k
        )
