"""Type A plot for shared-trajectory J-lens additive gaps (v3)."""
...
def plot_type_a(
    ...
    *,
    ...
    fig, axes = plt.subplots(1, 2, figsize=(11.2, 4.4), gridspec_kw={"width_ratios": [1.25, 1.0]})
    ...
    ax.set_ylabel(r"J-lens tropical $d_M=-\ln\langle\pi_i,\pi_j\rangle$")
    ax.set_title("Shared-trajectory J-lens chain distances", fontsize=11)
    ...
    ax.axvline(float(np.mean(gaps)), color="#111", lw=1.4, label=f"mean = {np.mean(gaps):.2f}")
    note = (
        f"n = {n}\n"
    ...
    ax.set_ylabel("count")
    ax.set_title("J-lens shared-trajectory additivity", fontsize=11)
    ...
    fig.suptitle(
        f"Type A v3 — Shared trajectory + J-lens   ·   Gemma-2-2B layer {layer_index}",
        fontsize=12.5,
        fontweight="bold",
        y=1.02,
    )
